...
export class Value<T> {
  ...
  private collect(result: bool, codeInfoIndex: number, actualValue: string, expectValue: string): void {
    assertResult.collectCheckResult(this.reversed ? !result : result, codeInfoIndex, actualValue, expectValue);
  }
  ...
  isNull(codeInfoIndex: u32 = EXPECT_MAX_INDEX): Value<T> {
    this.collect(isNull<T>(this.data), codeInfoIndex, toJson(this.data), "to be null");
    return this;
  }
  notNull(codeInfoIndex: u32 = EXPECT_MAX_INDEX): Value<T> {
    this.collect(!isNull<T>(this.data), codeInfoIndex, toJson(this.data), "notNull");
    return this;
  }

  equal(checkValue: T, codeInfoIndex: u32 = EXPECT_MAX_INDEX): Value<T> {
    this.collect(equal<T>(this.data, checkValue), codeInfoIndex, toJson(this.data), "= " + toJson(checkValue));
    return this;
  }
  ...
  greaterThan(checkValue: T, codeInfoIndex: u32 = EXPECT_MAX_INDEX): Value<T> {
    this.collect(this.data > checkValue, codeInfoIndex, toJson(this.data), " > " + toJson(checkValue));
    return this;
  }
  ...
  closeTo(checkValue: T, delta: number, codeInfoIndex: u32 = EXPECT_MAX_INDEX): Value<T> {
    ...
    if (isFloat<T>(checkValue) && isFloat<T>(data)) {
      this.collect(abs(data - checkValue) < delta, codeInfoIndex, toJson(this.data), " closeTo " + toJson(checkValue));
    } else {
