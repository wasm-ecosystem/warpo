...
void trace(uint32_t ptr, uint32_t n, double d1, double d2, double d3, double d4, double d5,
...
int main() {
  ...
  std::vector<NativeSymbol> linkedApi{
      ...
      vb::NativeSymbol{vb::NativeSymbol::Linkage::STATIC, "binaryen", "_BinaryenLiteralInt64", "(Iii)",
                       reinterpret_cast<void *>(&BinaryenLiteralInt64)},
      vb::NativeSymbol{vb::NativeSymbol::Linkage::STATIC, "binaryen", "_BinaryenConst", "(II)I",
                       reinterpret_cast<void *>(&BinaryenConst)},
      vb::NativeSymbol{vb::NativeSymbol::Linkage::STATIC, "binaryen", "_BinaryenLiteralInt32", "(Ii)",
                       reinterpret_cast<void *>(&BinaryenLiteralInt32)},
  ...
  m.initFromBytecode(Span<const uint8_t>{embed_asc_wasm.data(), embed_asc_wasm.size()},
                     Span<NativeSymbol const>{linkedApi.data(), linkedApi.size()});

  uint8_t const *stackTop = static_cast<uint8_t const *>(getStackTop());
  m.start(stackTop);
  ...
  auto const allocString = [&m, stackTop](std::string_view str) -> int32_t {
  ...
  m.callExportedFunctionWithName<0>(stackTop, "parse", program, allocString("let v = 0;"), allocString("demo.ts"), 1);

  int32_t const compiled = m.callExportedFunctionWithName<1>(stackTop, "compile", program)[0].i32;

  WasmModule::destroyEnvironment();
}
