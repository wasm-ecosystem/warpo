...
void addSubProgram(uint32_t const subProgramNamePtr, uint32_t const belongClassNamePtr,
                   uint32_t const outerFunctionNamePtr, uint32_t const sourcePathPtr, uint32_t const startLine,
                   uint32_t const endLine, vb::WasmModule const *const ctx) {
  ...
  std::string const sourcePath = WarpRunner::getString(ctx, sourcePathPtr);
  FrontendCompiler *const pCompiler = static_cast<FrontendCompiler *>(ctx->getContext());
