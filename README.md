...
The following charts demonstrate the performance of WARP. The benchmarks that were used can be found here: [wasm-compiler-benchmarks](https://atc-github.azure.cloud.bmw/CDCFW/wasm-compiler-benchmark). All benchmarks were executed on an M1 MacBook Pro with 16GB of RAM using [scripts/run_bench.py](scripts/run_bench.py). The resulting score is the geometric mean of all benchmark times. V8 Liftoff and V8 TurboFan are the baseline (singlepass) and optimizing compiler variants used in Google Chrome and SpiderMonkey (SM) Baseline and Ion are the respective compiler used in Mozilla Firefox. Native execution represents the performance of a native release (`-O3`) build of the benchmark logic using Clang. Note that this is not relevant for the compilation speed benchmark.

Compilation and Execution speed were benchmarked separately to highlight the strengths and weaknesses of each candidate. It can be seen that the baseline compiler variants are significantly faster during compilation but produces less efficient code. Note that WARP is the fastest one that was tested and at the same time is only marginally slower than native execution.

Single threaded translation without lazy compilation.

```
python3 scripts/benchmark/run_bench.py -i ../wasm-compiler-benchmark -x build/bin/vb_bench -n 10
```
...
Tested with analytics `scripts/run_analysis.py` during compilation of an exemplary benchmark WebAssembly usecase module from the [wasm-compiler-benchmarks](https://atc-github.azure.cloud.bmw/CDCFW/wasm-compiler-benchmark) repo using the AArch64 variant with MMU support enabled. This benchmark did not measure the stack needed by the compiler. Due to the highly efficient and compressed nature of WebAssembly, the output machine code is inevitably larger than the input WebAssembly module.
