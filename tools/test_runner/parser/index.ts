...
export class Parser {
  ...
  async parse(wasmModule: WebAssemblyModule, sourceCodePaths: string[]): Promise<FileCoverageResult[]> {
    await this.traceParse(wasmModule);
   ...
   * @param debugInfoFile debugInfo file path generated by instrumentation
   */
  async traceParse(instrumentResult: WebAssemblyModule) {
    const [tempCovTraceMap, { debugInfos, debugFiles }] = await Promise.all([
      this.getTempCovTraceMap(instrumentResult.traceFile),
      this.getDebugInfos(instrumentResult),
    ]);
