...
export class SingleFunctionCoverageAnalysis {
    ...
    result;
    branchGraph = new Map();
    notFullyCoveredBasicBlock = new Set();
    constructor(covInfo, name) {
        ...
        let maxLine = Number.MIN_SAFE_INTEGER, minLine = Number.MAX_SAFE_INTEGER;
    ...
    updateBranch(indexSerialInSingleFunction) {
    ...
    markCoveredBranches(indexSerialInSingleFunction) {
        for (let i = 1; i < indexSerialInSingleFunction.length; i++) {
            const first = indexSerialInSingleFunction[i - 1], second = indexSerialInSingleFunction[i];
...
//# sourceMappingURL=singleFunctionAnalysis.js.map