 ...
 */
export function isFunctionInsideFile(fileName, functionName) {
    // `/`: parent is File
...
export function checkGenerics(functionName) {
...
export function checkVarargs(functionName) {
    if (functionName.endsWith("@varargs")) {
        return functionName.slice(0, -8);
    }
    return undefined;
}
export function supplyDefaultFunction(infos, importObject, importsArg) {
    for (const info of infos) {
        ...
        if (module === "env" && name === "abort") {
            importObjectModule[name] = (msg, file, line, col) => {
                const exports = importsArg.exports;
                throw new WebAssembly.RuntimeError(`abort: ${exports.__getString(msg)} at ${exports.__getString(file)}:${line}:${col}`);
            };
        }
        else if (module === "env" && name === "trace") {
            importObjectModule[name] = (msg, n, ...args) => {
                const exports = importsArg.exports;
            ...
            importObjectModule[name] = (..._args) => {
                return info.return?.kind === -2 /* TypeKind.i64 */ ? BigInt(0) : 0;
            };
        }
    }
}
