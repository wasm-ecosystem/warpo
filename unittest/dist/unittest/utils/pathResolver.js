...
export function splitCommand(cmdline) {
    ...
    const cmd = res[0];
    if (cmd === undefined)
        throw new Error(`Invalidat Command: ${cmdline}`);
    const argv = res.slice(1);
    return { cmd: cmd, argv: argv };
}
function getAllFileInFolder(folder, filter = () => true) {
    const res = [];
    let sub;
    try {
...
export function findRoot(filePaths) {
...
export function getIncludeFiles(includePatterns, filter) {
