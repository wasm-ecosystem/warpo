...
import { Rate, OrganizationName, Repository } from "../../interface.js";
...
function generateFileInfo(fileInfo) {
    const { filename, statementCoverageRate: statement, branchCoverageRate: branch, functionCoverageRate: func, lineCoverageRate: line, } = fileInfo;
    return `
<tr>
...
function generateFileInfos(filesInfo) {
    return filesInfo.map((v) => generateFileInfo(v)).join("\n");
}
export function generateFolderHtml(relativePathofRoot, filesInfo) {
    const statementCoverageRate = new Rate();
    const branchCoverageRate = new Rate();
    const functionCoverageRate = new Rate();
    const lineCoverageRate = new Rate();
...
//# sourceMappingURL=genFolder.js.map