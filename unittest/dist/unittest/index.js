...
export function validateArgument(includes, excludes) {
...
async function startUniTestImpl(options) {
    const failurePath = join(options.outputFolder, "failures.json");
    let failedTestCases = [];
    if (options.onlyFailures) {
        failedTestCases = JSON.parse(readFileSync(failurePath, "utf8"));
        if (failedTestCases.length === 0) {
            options.collectCoverage = true;
            console.log(chalk.yellowBright('Warning: no failed test cases found while enabled "onlyFailures", execute all test cases by default'));
        }
    }
...
export async function start_unit_test(options) {
    try {
