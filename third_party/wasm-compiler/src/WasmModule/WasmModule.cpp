...
void WasmModule::initFromBytecode(Span<uint8_t const> const &bytecode, Span<NativeSymbol const> const &linkedFunctions) {
  CompileResult const compileResult{compile(bytecode, linkedFunctions)};

  setupRuntime(compileResult.getModule().span(), linkedFunctions, compileResult.getDebugSymbol().span());
}
...
WasmModule::CompileResult WasmModule::compile(Span<uint8_t const> const &bytecode, Span<NativeSymbol const> const &linkedFunctions,
                                              bool const enableUnknownImports, bool const highPressureMode) {
  // coverity[autosar_cpp14_a16_2_3_violation] fake positive
  Compiler compiler{&compilerRealloc, &compilerMemoryAllocFnc, &compilerMemoryFreeFnc, this, &jitRealloc, enableUnknownImports};
...
void WasmModule::initFromCompiledBinary(Span<uint8_t const> const &compiledBinary, Span<NativeSymbol const> const &linkedFunctions,
  ...
  setupRuntime(compiledBinary, linkedFunctions, rawDebugSymbol);
}
...
void WasmModule::runtimeMemoryAllocFnc(ExtendableMemory &currentObject, uint32_t minimumLength) VB_NOEXCEPT {
  // GCOVR_EXCL_START
  assert(reallocFunction_ != nullptr && "Realloc function is not set");
  if (reallocFunction_ == nullptr) {
    return;
  ...
  if (minimumLength == 0U) {
    ...
    if (minimumLength <= maxRam_) {
      ...
      minimumLength = static_cast<uint32_t>(std::min(effectiveLimit, proposedLength));
