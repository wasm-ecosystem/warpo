...
Stack::iterator Common::findBaseOfValentBlockBelow(Stack::iterator belowIt) const {
  Stack::iterator const frameBase{getCurrentFrameBase()};
  uint32_t subunitsToSkip{1U};
  while (subunitsToSkip > 0U) {
    if (belowIt == frameBase) {
      throw ValidationException(ErrorCode::findBaseOfValentBlock__Stack_frame_underflow);
    }
    --belowIt;
    subunitsToSkip--;
...
Stack::iterator Common::condenseValentBlockBelow(Stack::iterator const belowIt, StackElement const *const enforcedTarget) const {
  if (belowIt == compiler_.stack_.begin()) {
    throw ValidationException(ErrorCode::condenseValentBlockCore__Stack_frame_underflow);
  }
...
Stack::iterator Common::skipValentBlock(uint32_t const count) const {
