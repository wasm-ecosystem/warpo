import lauterbach.trace32.rcl as t32
from lauterbach.trace32.rcl._rc._symbol import Symbol
from lauterbach.trace32.rcl._rc._breakpoint import Breakpoint
from lauterbach.trace32.rcl._rc._error import ApiConnectionTimeoutError
import os

logDir = os.getenv("VB_SPECTEST_LOG_DIR", ".")

generateBinaryAndBackupFailedModules = True
compileNatively = False


class LauterbachSpectest:

    def __init__(self, logDir: str) -> None:

        self.__symbolCache: dict[str, Symbol] = {}
        self.__logfile = open(os.path.join(logDir, "spectest.log"), "w")

        self.__dbg = t32.connect(
            node="localhost", port=20000, protocol="TCP", timeout=3.0
        )
        # self.__flashTestElf()

        self.__initBreakpoint()

    def processBreakPoint(self) -> None:
        messageSize = self.getIntByVariableName("TEST_MESSAGE_SIZE")
        if messageSize > 0:
            message = self.getBytesByVariableName("TEST_MESSAGE", messageSize).decode(
                "ascii"
            )
            self.__logfile.write(message)
            self.__logfile.flush()
            print(message)

    def getIntByVariableName(self, name: str) -> int:
        symbol: Symbol = self.__getSymbolByVariableName(name)
        value: int = 0
        if symbol.size == 1:
            value = self.__dbg.memory.read_uint8(symbol.address)
        elif symbol.size == 2:
            value = self.__dbg.memory.read_uint16(symbol.address)
        elif symbol.size == 4:
            value = self.__dbg.memory.read_uint32(symbol.address)
        else:
            raise Exception("invalid symbol size")
        return value

    def getAddressByVariableName(self, name: str) -> int:
        symbol = self.__getSymbolByVariableName(name)
        return symbol.address.value

    def getBytesByVariableName(self, variableName: str, size: int) -> bytes:
        symbol = self.__getSymbolByVariableName(variableName)
        memoryData = self.__dbg.memory.read_uint8_array(
            address=symbol.address, length=size, width=1
        )

        return memoryData.tobytes()

    def setMemoryByAddress(self, addressInt: int, data: bytes) -> None:
        address = self.__dbg.address(access="D", value=addressInt)
        int_tuple = tuple(int(byte) for byte in data)
        self.__dbg.memory.write_uint8_array(address=address, data=int_tuple)

    def startTest(self) -> None:
        while True:
            state = self.__dbg.get_state()
            if len(state) > 0:
                stateValue = state[0]

                if stateValue == 2:  # break point hit
                    testDone = self.getIntByVariableName("TEST_DONE")
                    if testDone == 0:
                        self.processBreakPoint()
                        self.__dbg.go()
                    else:
                        break

    def __getSymbolByVariableName(self, variableName: str) -> Symbol:
        symbol = self.__symbolCache.get(variableName)

        if symbol == None:
            symbol = self.__dbg.symbol.query_by_name(variableName)
            self.__symbolCache[variableName] = symbol
        return symbol

    def __flashTestElf(self):
        projectRootPath = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        cmmFilePath = os.path.join(projectRootPath, "cmm", "spectest_flash.cmm")
        try:
            self.__dbg.cmm(cmd=cmmFilePath, timeout=100000)
        except ApiConnectionTimeoutError:
            pass  # Flash spectest need long time, the API may get socket error even if timeout is set. It's a bug caused by API internal implementation

    def __setDataBreakpoint(self, variableName: str) -> None:
        symbol = self.__getSymbolByVariableName(variableName)
        breakPointType: Breakpoint.Type = Breakpoint.Type(Breakpoint.Type.WRITE)
        self.__breakpoint = self.__dbg.breakpoint.set(
            address=symbol.address, type_=breakPointType
        )

    def __initBreakpoint(self) -> None:
        self.__setDataBreakpoint("TEST_DONE")
        symbol = self.__getSymbolByVariableName("DEBUGGER_NOTIFICATION")
        self.__breakpoint = self.__dbg.breakpoint.set(address=symbol.address)

    def disconnect(self):
        self.__logfile.close()
        self.__dbg.disconnect()


lauterbachFuzz = LauterbachSpectest(logDir)

lauterbachFuzz.startTest()

lauterbachFuzz.disconnect()
