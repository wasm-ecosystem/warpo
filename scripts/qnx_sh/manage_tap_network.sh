...
set -euo pipefail
# Set directory where data-dependencies can be found or fallback to be able to
# run without bazel as well
RUNFILES="$0.runfiles/ddad/platform/aas/tools/deployment/"
if [ -d "$RUNFILES" ]; then
    LIB_DIR=$RUNFILES
else
    LIB_DIR=$(dirname "$0")
...
print_usage () {
    echo "Usage:"
    echo "    $0"
    echo "            -m | --machine    machine for which the network should be configured (mpad_pp or mpad_high_pp)"
