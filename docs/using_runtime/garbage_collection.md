...
In optimzation lowering passes, WARPO will convert these 2 functions to the store operation in shadow stack.

The shadow stack area is static allocated in linear memory which is from `__stack_pointer` to `__heap_base`.
In the prelogue of each GC related function, `__stack_pointer` will decrease to allocate some space of shadow stack.
