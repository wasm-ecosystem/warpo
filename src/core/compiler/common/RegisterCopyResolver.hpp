...
public:
  ...
  void resolve(MoveEmitter const &moveEmitter, SwapEmitter const &swapEmitter) VB_THROW {
    ...
    if (remainingCounter > 0U) {
      ...
      // GCOVR_EXCL_STOP
      size_t swapIndex{0U};
      for (; swapIndex < static_cast<size_t>(cursor_); swapIndex++) {
        ResolverRecord const &record{records_[swapIndex]};
        if (isOperationalRecord(record)) {
          break;
        }
      }
      ...
      for (; remainingCounter > 1U; remainingCounter--) {
        ResolverRecord const &record{records_[swapIndex]};
        swapEmitter(record.target, record.source, swapContains64);
        swapIndex = getNextSwap(record.source.location.reg);
      }
    }
  }
...
private:
  ...
  size_t getNextSwap(TReg const targetReg) const VB_NOEXCEPT {
    ...
    // GCOVR_EXCL_START
    assert(false && "No operational record found for swap");
    UNREACHABLE(return 0U, "No operational record found for swap");
    // GCOVR_EXCL_STOP
  }
