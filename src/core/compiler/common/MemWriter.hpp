...
public:
  ...
  /// accordingly
  ///
  /// @tparam Type Explicit type which is intended to be written
  /// @tparam Source Given parameter type
  /// @param source Data which should be written
  /// @throws vb::RuntimeError If not enough memory is available
  // coverity[autosar_cpp14_a8_4_7_violation]
  template <class Type, class Source> inline void write(Source const &source) VB_THROW {
    static_assert(std::is_same<Type, Source>::value, "Tried to write wrong type");
    static_assert(std::is_trivially_copyable<Source>::value, "write requires the source type to be copyable");
    step(sizeof(Source));
    static_cast<void>(std::memcpy(pSubI(ptr(), sizeof(Source)), &source, sizeof(Source)));
  }
