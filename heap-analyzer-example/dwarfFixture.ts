...
let myTuple: [string, i32, Vector2] = ["tuple", 123, new Vector2(1.0, 2.0)];
...
export function _start(): void {
  ...
  // -- SmallTuple --
  const smallTuple: [string, Player, Vector2] = ["tuple", new Player(0, "TuplePlayer", 0, 0.0, 0.0), new Vector2(1.0, 2.0)];
  ...
  player2.addItem(new Item("Lockpick", 1));
  player2.score = 1500;
  ...
  playerMap.set("Alice", new Player(201, "Alice (map)", 100, 1.0, 2.0));
  playerMap.set("Bob", new Player(202, "Bob (map)", 85, -1.0, 3.0));

  // -- Dump memory to file via host import --
  const dumpPath = "./heap-analyzer-example/example-before.dump";
  ...
  const capturedPlayer = new Player(301, "ClosurePlayer", 77, 7.0, 8.0);
  capturedPlayer.score = 4200;
  function inner(): u32 {
    return capturedPlayer.score;
  }
